  ...
  async execute(interaction: ChatInputCommandInteraction) {
    try {
      ...
      if (interaction.deferred || interaction.replied) {
        await interaction.followUp({ content: "An error occurred while communicating with the database. Please contact the server administrator.", ephemeral: true });
      } else {
        await interaction.reply({ content: "An error occurred while communicating with the database. Please contact the server administrator.", ephemeral: true });
      }
    }
  },
};
